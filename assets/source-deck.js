/* 원자료 슬라이드 뷰어 — 장표 넘겨보기·전체 보기 격자·장표 위 영상 재생.
   면은 이 파일보다 먼저 window.DECK 을 선언한다. 계약 —
     dir      장표·썸네일이 있는 폴더 (필수, 예 "../../assets/deck/")
     vdir     로컬 영상 폴더 (슬라이드에 v[].f 가 있는 면만)
     chapters [{n,t,lo,hi}] · n 이 0 이 아니면 제목 앞에 번호를 붙인다
     slides   [{n,c,s,k[],v[]}] · c 는 chapters 의 0-based 색인
              v[] 는 {l,t,w,h} 좌표에 y(유튜브 ID) 또는 f(로컬 파일),
              x 가 있으면 재생본이 없는 자리라 건너뛴다 */
