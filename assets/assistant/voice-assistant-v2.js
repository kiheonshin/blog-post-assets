...
export class KiheonVoiceAssistant extends HTMLElement {
  ...
  render() {
    ...
    this.transcriptDetails = this.querySelector("[data-assistant-transcript-details]");
    this.panel = this.querySelector("[data-assistant-panel]");
    this.openButtons = [...this.querySelectorAll("[data-assistant-open],[data-assistant-open-voice]")];
    this.openButton = this.openButtons.find((button) => button.matches("[data-assistant-open]"))
      ?? this.openButtons[0];
  ...
  seriesMarkup() {
    return `
      <section class="voice-assistant__rail" aria-labelledby="${this.instanceId}-title">
        ...
        <div class="voice-assistant__rail-actions">
          <button type="button" class="voice-assistant__rail-cta" data-assistant-open-voice aria-expanded="false" aria-controls="${this.instanceId}-dialog">도슨트와 대화하기</button>
        </div>
      </section>
      ${this.dialogMarkup({
  ...
  conversationMarkup() {
    return `
        ...
        <div class="voice-assistant__voice-visual">
          ...
          <button class="voice-assistant__voice-session" type="button" data-assistant-voice aria-label="음성 대화 시작" title="음성 대화 시작" aria-pressed="false">
            <svg class="voice-assistant__mic-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
              ...
              <path d="M6.5 11.5a5.5 5.5 0 0 0 11 0M12 17v3.5M9.5 20.5h5"></path>
            </svg>
            <span class="voice-assistant__sr-only" data-assistant-voice-label>음성 대화 시작</span>
          </button>
        </div>
      ...
      <details class="voice-assistant__transcript-details" data-assistant-transcript-details>
        ...
        <div class="voice-assistant__transcript-body">
          <div class="voice-assistant__transcript" role="log" aria-live="polite" aria-label="안내 대화 기록">
            <div class="voice-assistant__turn" data-assistant-role="assistant" data-assistant-initial>
              ...
              <p class="voice-assistant__copy" data-assistant-transcript>음성으로 바로 대화하거나, 필요할 때 글로 질문해 주세요.</p>
            </div>
            <nav class="voice-assistant__targets" aria-label="관련 페이지" data-assistant-targets hidden></nav>
          </div>
        </div>
      </details>
      <div class="voice-assistant__text-entry" data-assistant-text-entry>
        <form class="voice-assistant__form" data-assistant-form>
          <div class="voice-assistant__composer">
            <label>
              <span>직접 질문하기</span>
              <textarea data-assistant-input autocomplete="off" maxlength="500" rows="1" placeholder="궁금한 점을 글로 적어 주세요"></textarea>
            </label>
          </div>
      ...
      <div class="voice-assistant__controls">
        <details class="voice-assistant__settings" open>
          <summary>
            <span class="voice-assistant__settings-heading">목소리·속도</span>
            <span class="voice-assistant__settings-current" aria-label="현재 음성 설정">
              <span class="voice-assistant__settings-badge" data-assistant-settings-summary>기본 목소리</span>
              <span class="voice-assistant__settings-badge" data-assistant-rate-summary>보통</span>
            </span>
          </summary>
          ...
          <button class="voice-assistant__preview" type="button" data-assistant-preview>현재 설정 미리 듣기</button>
        </details>
  ...
  voiceRadio(value, label, meta, checked) {
    ...
    copy.className = "voice-assistant__option-copy";
    name.className = "voice-assistant__option-name";
    name.textContent = label;
    detail.className = "voice-assistant__option-meta";
  ...
  updateSettingsSummary() {
    if (!this.settingsSummary || !this.rateSummary) return;
    ...
    this.rateSummary.textContent = rateLabel;
  }
  ...
  handleClick(event) {
    ...
    const openVoice = event.target.closest("[data-assistant-open-voice]");
    if (openVoice) {
      this.openPanel(openVoice, { focusTarget: "voice" });
      this.startVoice();
      return;
    }
    ...
    if (prompt) {
      if (this.scope === "series" && this.panel?.hidden) this.openPanel(promptButton, { focusTarget: "input" });
  ...
  handleAudioClaim(event) {
    if (event.detail?.owner !== this && (this.voiceSessionActive || this.dataset.state === "speaking")) {
      this.stopVoice({ quiet: true });
    }
  }
  ...
  setOpenButtonsExpanded(expanded) {
    const buttons = this.openButtons?.length
      ? this.openButtons
      : [this.openButton].filter(Boolean);
    for (const button of buttons) button.setAttribute("aria-expanded", String(expanded));
  }

  openPanel(button, { focusTarget = "voice" } = {}) {
    ...
    this.setOpenButtonsExpanded(true);
    this.updateSectionLabel();
    (focusTarget === "input" ? this.input : this.voiceButton)?.focus();
  }
  ...
  closePanel() {
    ...
    this.setOpenButtonsExpanded(false);
  ...
  setVoiceButtonState(active) {
    ...
    if (this.voiceButton.dataset) this.voiceButton.dataset.active = String(active);
  ...
  syncVoiceButtonLabel() {
    ...
    const copy = this.voiceButton.querySelector?.("[data-assistant-voice-label]");
