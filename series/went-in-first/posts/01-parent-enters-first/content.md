...
2020년 2월에는 프로젝트 미팅을 준비하며 예전 기록을 다시 꺼냈다. 4월에는 아이와 화면 밖에서 이어지는 놀이 도구를 시험했다. 직접적인 캠페인 기록은 6월 말부터 시작된다. 그 뒤 8월의 컨셉안, 12월의 CES 구조, 이듬해 1월의 미션과 소셜 운영으로 이어지면서 관심과 놀이 옆에 있던 활동이 해를 넘기는 장기 실행으로 구조화됐다.
...
날짜가 선명하다고 인과까지 선명해지는 것은 아니다. 2월 기록은 프로젝트를 준비했다는 사실을, 4월 기록은 화면 밖으로 놀이를 확장해 봤다는 사실을, 6월 이후 기록은 실행이 길게 이어졌다는 사실을 확인해 준다. 이 셋을 함께 놓으면 2020년 봄과 여름에 무엇이 가까이 있었는지는 보이지만, 무엇이 무엇을 낳았는지는 보이지 않는다. 순서를 지키는 일은 이 경험을 과장하지 않기 위한 첫 조건이다.

## 2. 두 판본은 서로 다른 질문에 답한다
...
4S를 공간으로 번역하는 일은 표어를 벽에 붙이는 것과 달랐다. 홈시네마에서는 무엇을 보게 할지, 스포츠 구역에서는 어떤 움직임을 떠올리게 할지, 게임 구역에서는 반응을 어떻게 체감하게 할지, 갤러리에서는 얇은 형태를 어떤 장면으로 보이게 할지를 정해야 했다. 네 단어가 네 구역이 되면서 방문자가 걸어 다닐 수 있는 동선과 활동으로 바뀌었다.
...
600시간만 떼어 놓으면 프로젝트 전체의 최종 수치처럼 보이고, 1,800시간만 떼어 놓으면 한 사람이 여섯 달 동안 투입한 노동시간처럼 보일 수 있다. 하지만 기록의 단위는 디바이스에 누적된 플레이 시간이다. 두 대가 네 대가 되었고 관측 시점도 약 3개월에서 약 6개월로 이동했다. 분모가 달라졌으므로 숫자는 반드시 조건과 함께 읽어야 한다.
...
소셜 피드와 영상은 섬을 설명하는 부속 홍보물이 아니었다. 섬을 처음 알리는 티저, 네 공간 테마를 각각 보여주는 영상, 들어오는 방법을 안내하는 튜토리얼, 그 안에서 지내는 장면과 만드는 과정을 보여주는 영상이 서로 다른 역할을 맡았다. 방문 전에는 무엇을 기대할지 알려 주고, 방문 중에는 무엇을 할지 안내하며, 방문 뒤에는 어떤 장면을 다시 꺼낼지 마련했다. 한 번의 방문을 앞뒤의 콘텐츠와 연결한 셈이다.
...
이 실행층이 98쪽 판본의 역할을 분명하게 만든다. 37쪽을 읽으면 어디를 어떻게 걸어 다니게 했는지가 보이고, 98쪽을 읽으면 그 공간을 CES와 미션과 소셜 운영에 어떻게 사용했는지가 보인다. 두 문서를 함께 놓아야 설계와 실행이 모두 보인다.

## 6. 일과 놀이와 배움이 가까이 있었다
...
이 제안 기록을 앞의 동물의 숲 캠페인과 같은 실행 사례로 놓을 수는 없다. 하나는 실제 섬을 만들고 운영한 결산이 있고, 다른 하나는 만들기 전에 여러 가능성을 비교한 설계 문서다. 다만 이용자로 먼저 들어가 본 경험 옆에, 만드는 쪽에서 자유와 확인의 조건을 함께 검토한 기록이 남아 있다는 점은 확인할 수 있다.
...
먼저 들어간다는 말은 낯선 세계를 모두 이해했다는 뜻이 아니다. 되돌리기 어려운 조건을 고르고, 공간을 나누고, 여러 기기로 운영하고, 방문자가 할 일을 설계하면서 그 세계의 제약을 직접 확인했다는 뜻에 가깝다. 무엇을 설명할 수 있고 무엇은 아직 모르는지가 그 과정에서 구체적으로 드러났다.
