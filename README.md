...
### 발행 push 전에 한 번 더 (외부 링크 착지)

참조 링크는 착지를 확인한 것만 싣는다. 스위트가 재는 것은 **파일만 보고 판정되는 절반**이다.

- `published-links.test.mjs` : 단축 링크 0 · `http://` 0. 단축 링크는 착지를 숨기기 때문에
  이 저장소에서는 쓰지 않는다. 훑기가 조용히 0 개를 읽어도 초록이 되지 않도록, 실제로 몇 개를
  읽었는지도 함께 못박는다.

나머지 절반(대상이 사라졌는가 · 다른 회사로 넘어갔는가)은 네트워크를 타므로 스위트에 두지
않는다. **발행 push 전에 운영 쪽 착지 감사 러너를 한 번 돌린다.** 봇을 막아 403 을 주는
호스트는 죽은 것이 아니므로 러너 옆 예외 목록에 호스트와 확인한 날짜를 적어 두고, 목록에
없는 403 은 사람이 브라우저로 한 번 열어 본 뒤에 판정한다.

죽은 참조는 조용히 지우지 않는다. 본문이 그 대상을 설명하는 자리라면 보존본(웹 아카이브)으로
바꾸고, 표시되는 주소에 원래 주소와 보존 시점을 적어 독자가 무엇을 언제 본 것인지 알게 한다.
